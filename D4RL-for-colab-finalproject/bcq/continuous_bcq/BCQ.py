...
class Critic(nn.Module):
	...
	def compute_cql_loss(self, state, action, alpha=0.1):
		...
		random_actions = torch.FloatTensor(state.shape[0] * 10, action.shape[1]).uniform_(-1, 1).to(state.device)
		random_states = state.unsqueeze(1).repeat(1, 10, 1).view(state.shape[0] * 10, state.shape[1])

		# Compute Q-values for random actions
		q1_rand, q2_rand = self.forward(random_states, random_actions)

		# Compute Q-values for current policy actions
		q1_current, q2_current = self.forward(state, action)

		# CQL Loss
		cql_loss = alpha * (torch.logsumexp(q1_rand, dim=1).mean() - q1_current.mean()) + \
  					alpha * (torch.logsumexp(q2_rand, dim=1).mean() - q2_current.mean())
...
class BCQ(object):
	...
	def train(self, replay_buffer, iterations, batch_size=100):
		...
		for it in range(iterations):
			...
			cql_loss_1 = self.cql_loss(current_Q1, action)
			cql_loss_2 = self.cql_loss(current_Q1, action)
			critic_loss = 0.5 * critic_loss + cql_loss_2 + cql_loss_1
