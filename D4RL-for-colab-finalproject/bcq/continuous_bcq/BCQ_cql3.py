...
class Critic(nn.Module):
	def __init__(self, state_dim, action_dim):
		...
		self.l4 = nn.Linear(state_dim, 800)
...
class BCQ(object):
	...
	def train(self, replay_buffer, iterations, batch_size=100):
		...
		for it in range(iterations):
			...
			logsumexp_Q1 = torch.logsumexp(current_Q1, dim=1)
			logsumexp_Q2 = torch.logsumexp(current_Q2, dim=1)
			# Q_a1 = current_Q1#.gather(1, action)
			# Q_a2 = current_Q1#.gather(1, action)
			lhs1 = alpha*((logsumexp_Q1 - Q_a1).mean())
			lhs2 = alpha*((logsumexp_Q2 - Q_a2).mean())
			
			bellman_er1 = F.mse_loss(current_Q1, target_Q)
			bellman_er2 = F.mse_loss(current_Q2, target_Q) 

			critic_loss = 0.5 * bellman_er1 + lhs1 + 0.5 * bellman_er2 + lhs2
