	...
	def train(self, replay_buffer, iterations, batch_size=100):
		...
		for it in range(iterations):
			...
			with torch.no_grad():
				...
				q1_next_curr_actions, q2_next_curr_actions = self.critic.forward(state, self.actor.forward(next_state, action))
				q1_pred_actions, q2_pred_actions           = self.critic.forward(state, action)
				q1_curr_actions, q2_curr_actions           = self.critic.forward(state, self.actor.forward(state, action))

				cat_q1=torch.cat(
				    [q1_pred_actions + q2_pred_actions,
            	        q1_curr_actions + q2_curr_actions, 
                     q1_next_curr_actions + q2_next_curr_actions], 1
				)
			...
			min_qf1_loss=min_qf1_loss-((q1_pred_actions + q2_pred_actions).mean()*alpha)
